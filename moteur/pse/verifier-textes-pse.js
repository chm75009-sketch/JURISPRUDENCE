/* Relire à la source les articles que le module cite.
   ...
   Usage : node verifier-textes-pse.js [AAAA-MM-JJ]                          */
const fs = require("fs");
...
for (const n of numeros) {
  ...
  const etat = !lisibles.length ? "illisible"
    ...
    : (lisibles[0].texte === net(attendu.texte) ? "concordant"
      : (lisibles[0].id === attendu.id ? "même version, texte différent" : "écart"));
...
fs.writeFileSync(__dirname + "/verification-textes-pse.json",
