...
q("Élections", "listesDeposees", "Pour chaque liste : inscrits femmes et hommes du collège, et sexe de chaque candidat dans l'ordre de dépôt", "liste d'objets", "listes déposées et liste électorale");
...
/* Vérification de non-divergence : tout contrôle doit être atteint par au moins
   une donnée du questionnaire, sinon une exigence serait contrôlée sans être
   demandée. */
...
note("Deux données n'alimentent aucun contrôle et c'est voulu : « entreprise » n'est qu'un intitulé repris dans le rapport, et « ues » déclenche la mention de revue professionnelle obligatoire en tête du rapport, non un contrôle de conformité.");
note("Cette annexe est produite par le même calcul que la colonne « contrôle attendu ». Si elle signale un écart, c'est que le questionnaire et les contrôles ont divergé : le rapport ne doit pas être diffusé en l'état.");

fs.writeFileSync("_quest_cse.js", "module.exports=" + JSON.stringify(A.D) + ";");
fs.writeFileSync("_quest_cse.json", JSON.stringify(A.D));
console.log(`${LIGNES.length} données · ${C.length} contrôles · atteints ${atteints.size} · orphelins ${orphelins.length ? orphelins.join(",") : "aucun"} · données sans contrôle ${[...new Set(sansControle)].length}`);
