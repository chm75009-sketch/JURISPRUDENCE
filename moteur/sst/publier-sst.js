/* Publier le module « santé, sécurité et conditions de travail » : rejouer la
...
const { LIGNES } = require("./questionnaire-sst.js");
const M = require("./moteur-sst.js");
const T = JSON.parse(fs.readFileSync(path.join(ICI, "textes-sst.json"), "utf8"));
...
const FICHIERS = fs.readdirSync(ICI)
  .filter(x => /\.js$/.test(x) && x !== "publier-sst.js" || /^(textes-sst|fiche-sst)\.json$/.test(x))
  .sort();
const empreintes = {};
...
const manifeste = {
  domaine: "santé, sécurité et conditions de travail",
  date: new Date().toISOString().slice(0, 10),
  empreinte,
  fichiers: empreintes,
  compteurs: {
    articlesLus: Object.values(T).filter(v => v && v.texte).length,
    articlesReprisDuModuleCSE: Object.values(T).filter(v => v && v.source).length,
    seuils: Object.keys(M.SEUILS).length,
    controles: C.length,
...
try {
  const V = JSON.parse(fs.readFileSync(path.join(ICI, "verification-textes-sst.json"), "utf8"));
  ...
  console.log("   (les textes n'ont pas été relus à la source : node verifier-textes-sst.js)");
}
