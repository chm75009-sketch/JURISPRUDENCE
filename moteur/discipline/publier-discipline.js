/* Publier le module « discipline et règlement intérieur » : rejouer la
   chaîne, mesurer, estampiller.
   ...
   Usage : node publier-discipline.js      */
...
const { LIGNES } = require("./questionnaire-discipline.js");
const M = require("./moteur-discipline.js");
const T = JSON.parse(fs.readFileSync(path.join(ICI, "textes-discipline.json"), "utf8"));
...
const FICHIERS = fs.readdirSync(ICI)
  .filter(x => /\.js$/.test(x) && x !== "publier-discipline.js" || /^(textes-discipline|fiche-discipline)\.json$/.test(x))
  .sort();
const empreintes = {};
...
const manifeste = {
  domaine: "discipline et règlement intérieur",
  date: new Date().toISOString().slice(0, 10),
  empreinte,
  fichiers: empreintes,
  compteurs: {
    articlesLus: Object.values(T).filter(v => v && v.texte).length,
    articlesReprisDuModuleSocial: Object.values(T).filter(v => v && v.source).length,
    natures: Object.keys(M.NATURES).length,
    controles: C.length,
...
try {
  const V = JSON.parse(fs.readFileSync(path.join(ICI, "verification-textes-discipline.json"), "utf8"));
  ...
  console.log("   (les textes n'ont pas été relus à la source : node verifier-textes-discipline.js)");
}
