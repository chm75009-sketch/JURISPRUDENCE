...
def construire(elements, chemin, auteur=None, titre=None):
    doc = Document()
    ...
    _proprietes(doc, auteur, titre)
    doc.save(chemin)
    return chemin


def _proprietes(doc, auteur=None, titre=None):
    """LES PROPRIÉTÉS DU FICHIER NE DISENT PAS PAR QUOI IL EST PASSÉ.

    python-docx signe ses fichiers : auteur « python-docx », commentaire
    « generated by python-docx », et une date de création de 2013 héritée de
    son modèle. Ces trois mentions se lisent dans Word, onglet Informations,
    et sortent du cabinet avec le document. Elles sont effacées ici une fois
    pour toutes, plutôt que dans chaque script qui appelle ce générateur.
    Relevé le 25 septembre 2026."""
    import datetime

    p = doc.core_properties
    p.author = auteur or ""
    p.last_modified_by = auteur or ""
    p.title = titre or ""
    p.comments = ""
    p.category = ""
    p.keywords = ""
    ...
    p.created = maintenant
    p.modified = maintenant
    p.revision = 1
